...
def new_game() -> Engine:
    ...
    engine.message_log.add_message(
        "Welcome to ma crib!", colour.welcome_text
    )
...
class MainMenu(input_handlers.BaseEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        for i, text in enumerate(
            ["[N] Start a new sesh", "[C] Continue last sesh", "[Q] Quit"]
        ):
            console.print(
                console.width // 2,
    ...
    def ev_keydown(
        ...
        if event.sym in (tcod.event.K_q, tcod.event.K_ESCAPE):
            ...
            try:
                ...
                return input_handlers.PopupMessage(self, "You ain't got no spare seshes!")
            except Exception as exc:
                traceback.print_exc() # Print ot stderr.
